function partialVolume3DSurfaceViz(partialVolFile,surfaceFile,voxelSize,imageModality,downSample,plotVol,origDicomPath, writeIV)
...
%           plotVol         = 1 plots partial volume, 0 plots point cloud of Volume (much faster)
...
Igray = find(im ~=0);
[x y z] = ind2sub(size(im),Igray);
volPts = [x * voxelSize(1),y * voxelSize(2),z * voxelSize(3)];
volPts = [volPts(:,2) volPts(:,1) volPts(:,3)];
volPts(:,3) = volPts(:,3) -(imgSize * voxelSize(3));
volPts = downsample(volPts,floor(length(volPts) / 500));
...
if plotVol == 1;
    ...
    plot3(volPts(:,1),volPts(:,2),volPts(:,3),'ko','MarkerFaceColor','r')
    hold on;
